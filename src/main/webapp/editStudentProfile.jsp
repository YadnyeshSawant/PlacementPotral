...
<title>Edit Student Profile - Campus Placement Portal</title>
<script
	...
	<div class="flex h-screen overflow-hidden">
		...
		<main class="flex-1 flex flex-col overflow-y-auto">
			...
			<div class="p-8 max-w-5xl mx-auto w-full">
				<form action="EditStudentProfile"
					...
					<div
						...
						<div class="absolute -bottom-16 left-10">
							<div
								class="group relative w-32 h-32 rounded-2xl border-4 border-white bg-white overflow-hidden shadow-md">
								...
								<label
									class="absolute inset-0 bg-black/40 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity">
									<span class="material-symbols-outlined">photo_camera</span> <input
									accept="image/*" class="hidden" type="file" />
								</label>
							</div>
						</div>
					</div>
					<div
						...
						<div>
							<h2 class="text-3xl font-extrabold text-slate-900">Edit
								Student Profile</h2>
							<p class="text-slate-500 font-medium mt-1">Update your
								information for recruitment cycle 2024</p>
						</div>
						<div class="flex items-center gap-3">
							<a href="studentProfile"><button
									class="px-6 py-2.5 border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-xl text-sm font-bold transition-all"
									type="button">Cancel</button></a>
							<button
								class="flex items-center gap-2 px-6 py-2.5  bg-green-700 text-white rounded-xl text-sm font-bold transition-all shadow-md shadow-primary/10"
								type="submit">
					...
					<div class="px-10 pb-12 space-y-12">
						<section>
							...
							<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
								<div>
									...
									<input name="prn" value="${student.prn}"
										class="w-full text-slate-900 font-semibold p-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white"
										placeholder="Enter PRN" type="text" />
								</div>
								<div>
									<label
										class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">First
										Name</label> <input name="firstname" value="${student.firstName}"
										class="w-full text-slate-900 font-semibold p-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white"
								...
								<div>
									<label
										...
										ID</label> <select name="program"
										class="w-full text-slate-900 font-semibold p-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white appearance-none">
										<option value="2001"
											${student.programId == 2001 ? 'selected' : ''}>MCA</option>
										<option value="2002"
											${student.programId == 2002 ? 'selected' : ''}>BCA</option>
										<option value="2003"
											${student.programId == 2003 ? 'selected' : ''}>MBA</option>
									</select>
								</div>
							</div>
						</section>
						<section>
							...
							<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
								<div class="lg:col-span-1">
									...
									<label
										class="flex flex-col items-center justify-center gap-2 p-3 bg-slate-50 border border-dashed border-slate-300 rounded-xl group cursor-pointer hover:bg-blue-50/50 hover:border-primary transition-colors">
										<div class="flex items-center gap-3 w-full">
											<span
												class="material-symbols-outlined text-slate-400 group-hover:text-primary">upload_file</span>
											<span class="text-sm font-bold text-slate-700 truncate">Alex_Johnson_CV_2024.pdf</span>
											<span
												class="text-[10px] font-bold bg-slate-200 text-slate-600 px-2 py-0.5 rounded ml-auto group-hover:bg-primary group-hover:text-white">Change</span>
										</div> <input accept=".pdf,.doc,.docx" class="hidden" type="file"
										disabled="disabled" />
									</label>
								</div>
								<div>
									...
									<div class="relative">
										<span
											class="material-symbols-outlined absolute left-3 top-3.5 text-slate-400 text-sm">auto_awesome</span>
										<input name="sscPercentage"
											class="w-full text-slate-900 font-bold p-3 pl-9 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white"
											placeholder="00.00" step="0.01" type="number"
											value="${student.sscPercentage}" />
									</div>
								</div>
								<div>
									<label
								...
								<div>
									...
									<div class="relative">
										<span
											class="material-symbols-outlined absolute left-3 top-3.5 text-primary text-sm">stars</span>
										<input name="ugcgpa"
											class="w-full text-primary font-bold p-3 pl-9 bg-primary/5 border border-primary/10 rounded-xl focus:bg-white"
											placeholder="0.00" step="0.01" type="number"
											value="${student.bachelorCgpa}" />
									</div>
								</div>
								<div>
									<label
										class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Post
										Grad CGPA (Optional)</label>
									<div class="relative">
										<input name="pgcgpa"
											class="w-full text-slate-900 font-bold p-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white"
											placeholder="Enter CGPA" step="0.01"
											value="${student.postgradCgpa}" />
									</div>
								</div>
							</div>
						</section>
						<section>
							...
							<div>
								<label
									class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Technical
									Skills (Comma separated)</label>
								<textarea name="skills"
									class="w-full text-slate-900 font-semibold p-4 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white resize-none"
									placeholder="e.g. Java, Python, SQL, Web Development, React.js, Node.js"
									rows="3">Java, Python, SQL, Web Development, React.js, Node.js, Cloud Computing, Agile Methodology</textarea>
								<p class="mt-2 text-[10px] text-slate-400 font-medium italic">Separate
									each skill with a comma to generate profile tags.</p>
							</div>
						</section>
					</div>
				</form>
